template <typename T> void cpuFlux(T *f, T *xdg, T *udg, T *odg, T *wdg, T *uinf, T *param, T time, int modelnumber, int ng, int nc, int ncu, int nd, int ncx, int nco, int ncw)
{
	for (int i = 0; i <ng; i++) {
		...
		T x3 = 0.5*udg4;
		f[0*ng+i] = udg1*x1;
		f[1*ng+i] = 0.66666666666666663*param14*x1/xdg1 + pow(udg2, 2)*x0 - 1.3333333333333333*x0*sqrt(x2)*(-udg5*x0 + x1*x3)/sqrt(param1*param2) + udg3/param1;
		f[2*ng+i] = udg2*x2 - x0*pow(x2, 0.75)*sqrt(param1/param2)*(-udg6*x0 + x2*x3)/param3;
	}
}
