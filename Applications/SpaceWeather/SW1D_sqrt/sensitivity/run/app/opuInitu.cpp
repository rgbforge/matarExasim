template <typename T> void opuInitu(T *f, T *xdg, T *uinf, T *param, int modelnumber, int ng, int ncx, int nce, int npe, int ne)
{
	for (int i = 0; i <ng; i++) {
		...
		T x0 = param10 - xdg1;
		T x1 = (1.0/35000.0)*param12;
		T x2 = param9 - (-param8 + param9)*exp(x0*x1);
		T x3 = 1.0/param12;
		T x4 = 1.0/param9;
		T x5 = param10*pow(param4, 2) - 1;
		T x6 = log(1 + param9*(-1 + exp(-x0*x1))/param8);
		f[j+npe*0+npe*nce*k] = 35000*x3*x4*x5*x6 - log(x2);
		f[j+npe*1+npe*nce*k] = 0.0;
		f[j+npe*2+npe*nce*k] = x2*sqrt(exp(35000*x3*x4*x5*x6)/x2);
	}
}
