...
# save samples [A, B]
np.save(file="../SA_results/CR2053/" + str(folder) + "/samples/A_sample_scaled_" + str(N), arr=A_scaled)
np.save(file="../SA_results/CR2053/" + str(folder) + "/samples/B_sample_scaled_" + str(N), arr=B_scaled)

# generate matrix C=[C1, C2, C3, ..., C11].
C = generate_tensor_c(A=A_scaled, B=B_scaled, d=d)
np.save(file="../SA_results/CR2053/" + str(folder) + "/samples/C_sample_scaled_" + str(N), arr=C)

# generate matrix D=[D1, D2, D3, ..., D11].
D = generate_tensor_d(A=A_scaled, B=B_scaled, d=d)
np.save(file="../SA_results/CR2053/" + str(folder) + "/samples/D_sample_scaled_" + str(N), arr=D)
...
plt.tight_layout()
plt.savefig("figs/random_samples_" + str(N) + ".png", dpi=500)
plt.show()
